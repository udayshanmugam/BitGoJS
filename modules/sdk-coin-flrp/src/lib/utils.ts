...
export class Utils implements BaseUtils {
   ...
   */
  isTransferableOutput(output: Output): output is avm.TransferableOutput | platformvm.TransferableOutput {
  ...
  deprecatedMapOutputToEntry(network: FlareNetwork): (DeprecatedOutput) => Entry {
    return (output: DeprecatedOutput) => {
      if (this.deprecatedIsTransferableOutput(output)) {
        ...
        try {
          const transferableOutput = output as unknown as avm.TransferableOutput;
          const amount = (transferableOutput.getOutput() as avm.AmountOutput).getAmount();
  ...
  mapOutputToEntry(network: FlareNetwork): (Output) => Entry {
    return (output: Output) => {
      if (this.isTransferableOutput(output)) {
        const transferableOutput = output as avm.TransferableOutput | platformvm.TransferableOutput;
        const outputAmount = (transferableOutput.getOutput() as avm.AmountOutput).getAmount();
